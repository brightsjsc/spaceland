                            ...
                            @if ($filter <> '')
                              @foreach ($districts as $district)
                                <option value="{{ $district->district_id }}"
                                  @if ($filter['adr_district_id'] == $district->district_id)
                                    selected
                                  @endif
                                >
                            ...
                            @if ($filter <> '')
                              @foreach ($communes as $commune)
                                <option value="{{ $commune->commune_id }}"
                                  @if ($filter['adr_commune_id'] == $commune->commune_id)
                                    selected
                                  @endif
                                >
                ...
                <tbody>
                  @foreach($products as $product)
                    <tr>
                      <td>{{ $loop->index + 1 }}</td>
                      <td>
                        <img src="{{ asset('uploads/images/products/thumbs/'.$product['image']) }}" alt="" width="100px"><br>
                      </td>
                      <td><a href="{{ route('admin.product.edit',['id' => $product['id']]) }}"><b>{{ $product['name'] }}</b></a></td>
                      ...
                      <td style="width: 100px">{{ date('d/m/Y', strtotime($product['created_at'])) }}</td>
                      <td class="td-actions text-center">
                        @if ($product['status'] == 0)
                      ...
                      <td class="td-actions text-center">
                        <a href="{{ route('admin.product.edit',['id' => $product['id']]) }}" class="btn btn-link btn-success btn-just-icon" title="Sửa">
                          <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                        </a>
                        <a data-url="{{ route('admin.product.delete',['id' => $product['id']]) }}" onclick="return confirmSubmitDel()" class="btn btn-link btn-danger btn-just-icon remove" title="Xóa">
