    ...
    <form action="{{ route('contact') }}" method="post" id="{{ $idFormRandom }}">
        ...
        <div class="form-group">
            <input type="text" placeholder="Họ tên" class="form-control
                  @error('name') is-invalid @enderror" name="name" id="fullname" required>
            @error('name')
                <span><small>{{ $message }}</small></span>
            @enderror
        </div>
        <div class="form-group">
            <input type="phone" placeholder="Số điện thoại" class="form-control  @error('phone') is-invalid @enderror"
                name="phone" id="phoneNumber" required>
            @error('phone')
                <span><small>{{ $message }}</small></span>
            @enderror
        </div>
        <div class="form-group">
            <select name="id_district" class="form-control  @error('id_district') is-invalid @enderror"
                id="district_name" required>
                <option value="" selected disabled hidden>Khu vực</option>
