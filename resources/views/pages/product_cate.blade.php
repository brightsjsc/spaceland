                            ...
                            <div class="product-item row">
                                ...
                                <div class="product-image col-5">
                                    <a href="{{ route('productDetail', ['product' => $product->alias]) }}"><img
                                            ...
                                            alt="{{ $product->name }}" title="{{ $product->name }}" width="100%"></a>
                                </div>
            ...
            $("select[name='adr_district_id']").change(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var hasCommune = $("select[name='adr_commune_id']").attr(
                            'data-has-data') == 'true';
                        var idSelected = $("select[name='adr_commune_id']").attr(
                            'data-id-selected') || '-1';
                        ...
                        $.each(data, function(key, value) {
                            $("select[name='adr_commune_id']").append(
                                "<option value='" + value.commune_id + "' " + (
                                    idSelected == value.commune_id ? "selected" : ""
                                    ) + ">" + value.level + " " + value.name_local +
                                "</option>"
                            );
                        });
                        if (hasCommune) {
            ...
            $("select[name='adr_commune_id']").change(function() {
                ...
                if (commune_id == '') {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            var idSelected = $("select[name='project_id']").attr(
                                'data-id-selected') || '-1';
                            $("select[name='project_id']").html('');
                            $("select[name='project_id']").append(
                                "<option value=''> Chọn dự án </option>");
